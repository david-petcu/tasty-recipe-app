  ...
  <div class="page-shell page-section max-w-6xl">
    ...
    <article v-else>
      <header class="panel overflow-hidden">
        ...
        <div class="px-6 py-9 sm:px-10 sm:py-11"><div class="max-w-4xl"><div class="mb-5 flex flex-wrap items-center gap-2"><span class="badge-accent">{{ difficultyLabel[recipe.difficulty] || recipe.difficulty }}</span><span class="badge">{{ categoryLabel[recipe.category] || recipe.category }}</span><span class="flex items-center gap-2 text-sm text-slate-400"><UiIcon name="book" :size="16" />Community recipe</span><span v-if="reviewResponse?.total" class="flex items-center gap-1.5 text-sm font-semibold text-amber-300"><UiIcon name="star" :size="16" filled />{{ reviewResponse.average.toFixed(1) }} · {{ reviewResponse.total }} reviews</span></div><h1 class="max-w-3xl text-3xl font-semibold leading-tight text-stone-100 sm:text-5xl">{{ recipe.title }}</h1><p class="mt-4 max-w-2xl text-base leading-7 text-slate-400">{{ recipe.description || 'A recipe ready to be discovered and enjoyed.' }}</p><div class="mt-7 flex flex-wrap gap-2"><span class="badge px-3 py-2 text-sm"><UiIcon name="clock" :size="17" />{{ recipe.cooking_time }} minutes</span><span class="badge px-3 py-2 text-sm"><UiIcon name="servings" :size="17" />{{ recipe.servings }} servings</span><button type="button" class="button-secondary" :class="isSaved ? 'border-rose-800 bg-rose-950/40 text-rose-300' : ''" :disabled="savingFavorite" @click="toggleFavorite"><UiIcon name="heart" :size="17" />{{ savingFavorite ? 'Updating...' : isSaved ? 'Saved' : 'Save recipe' }}</button></div></div></div>
      </header>
