...
<template>
  <div class="pointer-events-none fixed right-4 top-24 z-[70] flex w-[min(24rem,calc(100vw-2rem))] flex-col gap-3" aria-live="polite">
    <TransitionGroup name="toast">
      <div v-for="item in notifications" :key="item.id" class="pointer-events-auto flex items-start gap-3 rounded-xl border bg-slate-900 p-4 shadow-2xl" :class="item.type === 'success' ? 'border-emerald-700 text-emerald-200' : item.type === 'error' ? 'border-red-800 text-red-200' : 'border-slate-700 text-slate-200'">
