...
<template>
  <div class="page-shell max-w-6xl">
    ...
    <template v-else>
      <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="p-6 sm:p-10">
          <div v-if="isOwnProfile" class="mb-6 flex items-center justify-between gap-4 border-b border-slate-800 pb-6">
            ...
            <NuxtLink to="/settings" class="button-secondary shrink-0"><UiIcon name="star" :size="17" />Account settings</NuxtLink>
          </div>
          <div class="flex flex-col items-start gap-6 sm:flex-row sm:items-center"><div class="flex h-32 w-32 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-slate-50 text-slate-600 ring-1 ring-slate-700"><img v-if="chef.chef_profiles?.profile_image" :src="chef.chef_profiles.profile_image" :alt="chef.username" class="h-full w-full object-cover" /><UiIcon v-else name="chef" :size="48" :stroke-width="1.3" /></div><div class="flex-1"><h1 class="text-4xl font-extrabold text-slate-900 sm:text-5xl">{{ chef.username }}</h1><div class="mt-3 flex flex-wrap gap-3"><span v-if="chef.chef_profiles?.location" class="flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1.5 text-sm font-bold text-slate-600"><UiIcon name="pin" :size="15" />{{ chef.chef_profiles.location }}</span><span class="flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1.5 text-sm font-bold text-slate-600"><UiIcon name="star" :size="15" />{{ chef.chef_profiles?.years_experience || 0 }} years of experience</span></div></div><div class="min-w-32 rounded-2xl bg-slate-50 px-6 py-4 text-center"><strong class="block text-3xl font-extrabold text-slate-900">{{ chef.recipes?.length || 0 }}</strong><span class="text-xs font-bold uppercase tracking-wider text-slate-400">Recipes</span></div></div>
      ...
      <section class="mt-14"><div class="mb-7 flex items-end justify-between"><div><p class="eyebrow">From the chef's kitchen</p><h2 class="mt-2 text-3xl font-extrabold tracking-tight text-slate-900">{{ chef.username }}'s recipes</h2></div><span class="hidden text-sm text-slate-400 sm:block">{{ chef.recipes?.length || 0 }} results</span></div>
